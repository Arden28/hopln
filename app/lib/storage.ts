...
const NativeDriver: KV | null = NativeAsync
  ? {
      async getItem(k) {
        return NativeAsync.getItem(k);
      },
      async setItem(k, v) {
        return NativeAsync.setItem(k, v);
      },
      async removeItem(k) {
        return NativeAsync.removeItem(k);
      },
    }
  : null;
...
export const Storage: KV =
  NativeDriver ?? (Platform.OS === "web" ? WebDriver : MemoryDriver);

// Optional: introspect which driver is active (handy while debugging)
export const storageDriver = NativeDriver
  ? "native-asyncstorage"
  : Platform.OS === "web"
    ? "web-localstorage"
    : "memory-fallback";
