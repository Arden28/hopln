...
export default function MapScreen() {
  ...
  function handleCamera(e: any) {
    ...
    const userMoved =
      (props.isUserInteraction ?? props.isGesture ?? false) && !navRef.current;
  ...
  function handleNavUpdate(nextSmoothed: Coords) {
    ...
    const targetZoom =
      NAV_ZOOM_BASE + (spd / NAV_ZOOM_SPD_MAX) * NAV_ZOOM_DELTA;
    ...
    screen = (
      <>
        <MapboxGL.MapView
          ...
          styleURL={STYLE}
          onRegionIsChanging={() => {
            if (!navRef.current) setFollowMe(false);
          }}
                ...
                nearest.map((s) => (
                  ...
                  >
                    ...
                    <Text style={styles.nearDist}>
                      {(s as any).dist < 1000
                        ? `${Math.round((s as any).dist)}m`
                        : `${((s as any).dist / 1000).toFixed(1)}km`}
                    </Text>
                  </Pressable>
                ))
              )}
            ...
            <View style={styles.searchCard}>
              ...
              <ScrollView style={{ maxHeight: 260 }}>
                {searchResults.length === 0 ? (
                  <Text style={[styles.sub, { paddingVertical: 8 }]}>
                    No results
                  </Text>
                ) : (
                  searchResults.map((s) => (
