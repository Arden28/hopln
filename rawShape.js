...
// This script reads 'shapes.csv' from the current directory and generates 'shapes.ts'

const data = [];

fs.createReadStream('shapes.csv')
  .pipe(csv())
  .on('data', (row) => {
...
export const shapes: Route[] = ${JSON.stringify(data, null, 2)};
    `;

    fs.writeFileSync('shapes.ts', tsContent);
